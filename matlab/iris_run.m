warning('off')
disp("Reading files");
D=readmatrix("../data/sonar/data.csv", "Delimiter",",");
disp("Running PaLD");
D=dist(D');
%D=D(2:end, 2:end);
...
writetable(T, "../data/sonar/matlab-results.csv");


disp("Running Prediction");
disp("Reading files");
D=readmatrix("../data/sonar/data.csv", "Delimiter",",");
G=readmatrix("../data/sonar/matlab-results.csv", "Delimiter",",");
P=readmatrix("../data/sonar/test_data.csv", "Delimiter",",");
...
T=table(rows(r),cols(r),vals(r));
T.Properties.VariableNames(1:3) = {'rows','cols','vals'};
writetable(T, "../data/sonar/predicted-cohesions.csv");
