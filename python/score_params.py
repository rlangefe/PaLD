...
if __name__ == '__main__':
    ...
    with cp.cuda.Device(0) as dev:
        ...
        X_train, X_test, y_train, y_test = train_test_split(df, groups, test_size=0.5, random_state=42)
        ...
        for name in pred_pairs:            
            ...
            results_dict[name.replace('_', ' ').title()] = [accuracy_score(y_test, Z, normalize=True), precision_score(y_test, Z, average='macro'), recall_score(y_test, Z, average='macro'), f1_score(y_test, Z, average='macro'), balanced_accuracy_score(y_test, Z)]
        ...
        for name in list_of_models:
            ...
            results_dict[name.replace('_', ' ').title().replace('Svc', 'SVC').replace('Knn', 'KNN')] = [clf.score(X_test, y_test), precision_score(y_test, clf.predict(X_test), average='macro'), recall_score(y_test, clf.predict(X_test), average='macro'), f1_score(y_test, clf.predict(X_test), average='macro'), balanced_accuracy_score(y_test, clf.predict(X_test))]
