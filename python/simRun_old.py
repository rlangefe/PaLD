...
from pald_cupy_multistream import pald as pald_cupy_multistream
from pald_cupy import pald as pald_cupy
from pald_cupy_loop import pald as pald_cupy_loop
from pald_cupy_custom import pald as pald_cupy_kernels
            ...
            for std1 in range(1, 80, 20):
                for dim in range(1,22,10):
                    for dist2 in ['normal', 'exponential', 'chisquare']:
                        for mean2 in range(0,200,75):
                            for std2 in range(1, 80, 20):
                                for n2 in range(10, 1001, 200):
                                    for n1 in range(10, 1001, 200):
                                        for num_times in range(t):                                 
                                            ...
                                            with cp.cuda.Device(0) as dev:
                                                dummy_kernel((10,), (10,), ())
                                                start_time = time.time()
                                                cohesions = pald_cupy(dist_mat, dev=dev)
                                                end_time = time.time()
                                                cohesions = cohesions.get()
                                                if interactive == True:
                                                    print('CuPy Normal')
                                                ...
                                                data_dict['time_pald_cupy'].append(end_time-start_time)
                                                data_dict['bound_pald_cupy'].append(cohesions.trace()/(dist_mat.shape[0]*2))
                                            ...
                                            with cp.cuda.Device(0) as dev:
                                                dummy_kernel((10,), (10,), ())
                                                start_time = time.time()
                                                cohesions = pald_cupy_kernels(dist_mat, dev=dev)
                                                end_time = time.time()
                                                cohesions = cohesions.get()
                                                if interactive == True:
                                                    print('CuPy Kernels')
                                                ...
                                                data_dict['time_pald_cupy_kernels'].append(end_time-start_time)
                                                data_dict['bound_pald_cupy_kernels'].append(cohesions.trace()/(dist_mat.shape[0]*2))
                                            ...
                                            pd.DataFrame(data=data_dict).to_csv(args.output, index=False)
