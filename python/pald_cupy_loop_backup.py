...
extern "C" __global__
void reach_set(const float* d_mat, const float *xy, const float *yx, float* Uxy, const int n, const int x) {
...
extern "C" __global__
void find_A3_x(const float* d_mat, const float* Uxy, float* A3_x, const float *sum_Uxy, const int n, const int x) {
    unsigned int tid_i = blockDim.x * blockIdx.x + threadIdx.x;
    unsigned int tid_j = blockDim.y * blockIdx.y + threadIdx.y;
...
def pald(D,block_size=32,dev=cp.cuda.Device(0)):
    ...
    for x in tqdm(range(0,n)):
        reach_set(((n//block_size)+1,(n//block_size)+1), (min(block_size,n),min(block_size,n)), (D_gpu, D_gpu[x,:], D_gpu[:,x], Uxy, cp.int32(n), x))
        sum_Uxy = reduce_Uxy(Uxy, axis=1)
